...
```{r}
library(axolotr)
create_credentials(GROQ_API_KEY = "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")
```
...
```{r}
payments <- read_csv("State_of_Maryland_Payments_Data__FY2008_to_FY2024_20250115.csv", guess_max=369008)
...
vendors_paid_capitol_project_UMD<- capitol_project_UMD |>
...
vendor_list <- as.list(vendors_paid_capitol_project_UMD$vendor_name)[1:100]
vendor_text <- paste(unlist(vendor_list), collapse = ", ")
```

```{r}
classquestion_response <- axolotr::ask(
  prompt = paste("You are comparing company names. Read the first 100 items in the vendor list below. Group vendor names that clearly refer to the same company, even if written in slightly different formats (e.g., with or without INC, LLC, punctuation, or spacing). Do NOT group names just because they start with the same few words. Only group them if they represent the same business entity. Ignore small spelling or punctuation differences (like commas or periods). Here is the vendor list:",vendor_text),
  model = "llama-3.1-8b-instant"
)

print(classquestion_response)
```

**Your comment: I modify the prompt several times, but still feel it doesn't do the cleaning well. And I also try to make it make the output each potential company in one line, but it doesn't seem to work.**
